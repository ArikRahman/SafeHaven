...
-opaque subject(DLQ) :: {subject, pid_(), gleam@dynamic:dynamic_()} |
    {named_subject, name(DLQ)}.

-type name(DLR) :: any() | {gleam_phantom, DLR}.

-type do_not_leak() :: any().

-type selector(DLS) :: any() | {gleam_phantom, DLS}.
...
-spec 'receive'(subject(DMQ), integer()) -> {ok, DMQ} | {error, nil}.
...
-spec receive_forever(subject(DMY)) -> DMY.
...
-spec selector_receive(selector(DNC), integer()) -> {ok, DNC} | {error, nil}.
...
-spec selector_receive_forever(selector(DNG)) -> DNG.
...
-spec map_selector(selector(DNI), fun((DNI) -> DNK)) -> selector(DNK).
map_selector(A, B) ->
...
-spec merge_selector(selector(DNM), selector(DNM)) -> selector(DNM).
merge_selector(A, B) ->
...
-spec select_map(selector(DNX), subject(DNZ), fun((DNZ) -> DNX)) -> selector(DNX).
...
-spec select(selector(DNT), subject(DNT)) -> selector(DNT).
...
-spec select_record(
    ...
    any(),
    integer(),
    fun((gleam@dynamic:dynamic_()) -> DOH)
) -> selector(DOH).
...
-spec select_other(selector(DOL), fun((gleam@dynamic:dynamic_()) -> DOL)) -> selector(DOL).
...
-spec deselect(selector(DOC), subject(any())) -> selector(DOC).
...
-spec select_specific_monitor(selector(DOX), monitor(), fun((down()) -> DOX)) -> selector(DOX).
...
-spec select_monitors(selector(DPA), fun((down()) -> DPA)) -> selector(DPA).
...
-spec select_trapped_exits(selector(DNQ), fun((exit_message()) -> DNQ)) -> selector(DNQ).
...
-spec deselect_specific_monitor(selector(DPD), monitor()) -> selector(DPD).
...
-spec send_after(subject(DPZ), integer(), DPZ) -> timer().
...
-spec perform_call(
    subject(DPG),
    fun((subject(DPI)) -> DPG),
    fun((selector(DPI)) -> {ok, DPI} | {error, nil})
) -> DPI.
...
-spec call(subject(DPN), integer(), fun((subject(DPP)) -> DPN)) -> DPP.
...
-spec call_forever(subject(DPR), fun((subject(DPT)) -> DPR)) -> DPT.
