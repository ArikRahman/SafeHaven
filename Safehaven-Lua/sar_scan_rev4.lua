...
function RunRemoteCommand(args)
    ...
    -- Using powershell.exe explicitly as requested
    
    -- The command we want to run on the remote machine:
    -- zsh -l -c 'cd <dir>; uv run <script> <args>'
    local remote_cmd_inner = string.format("cd %s; uv run %s %s", remote_dir, python_script, args)
    local remote_shell_cmd = string.format("zsh -l -c '%s'", remote_cmd_inner)
    
    -- The SSH command line (as it would appear in a PS terminal):
    -- ssh user@host "zsh -l -c '...'"
    -- We need to escape the double quotes around the remote command for the PS string
    local ssh_cmd_str = string.format("ssh %s \\\"%s\\\"", ssh_host, remote_shell_cmd)
    
    -- The full PowerShell command line:
    -- powershell -NoProfile -Command "ssh ... > 'log_file' 2>&1"
    local full_cmd = string.format("powershell -NoProfile -Command \"%s > '%s' 2>&1\"", ssh_cmd_str, log_file)
    
    WriteToLog("Gantry Command (PS): " .. args .. "\n", "black")
