...
def arcade_mode(initialX, initialY, step=STEP_PIXELS, chosen_margin=MARGIN_PIXELS, force_flag=False):
    ...
    continuous_dir = None
    repeat_interval = 0.05  # seconds between repeated moves while in continuous mode
    try:
        while True:
            k = read_key(0.1)
            if k is None:
            ...
            if k in ('w', 'W', 'k') or k == '\x1b[A' or k == '\x1b[OA':
                ...
                print('toggle continuous ->', continuous_mode)
                continue
            elif k in ('p', 'P'):
            ...
            # Do movement
            if continuous_mode:
                # Start continuous movement for the given direction
                continuous_dir = key
                # We will repeat moves until continuous_dir is changed or stopped
                while continuous_dir is not None:
                    # perform the move
                    if new_dx != 0 and new_dy != 0:
                    ...
                    # After a move, sleep a small interval and check keys
                    k2 = read_key(timeout=repeat_interval)
                    if k2 is None:
                        # keep repeating
                        continue
                    # If user pressed the same direction key, keep same direction
                    mapped = None
                    if k2 in ('w', 'W', 'k') or k2 == '\x1b[A' or k2 == '\x1b[OA':
                        mapped = 'up'
                    elif k2 in ('s', 'S', 'j') or k2 == '\x1b[B' or k2 == '\x1b[OB':
                        mapped = 'down'
                    elif k2 in ('a', 'A', 'h') or k2 == '\x1b[D' or k2 == '\x1b[OD':
                        mapped = 'left'
                    elif k2 in ('d', 'D', 'l') or k2 == '\x1b[C' or k2 == '\x1b[OC':
                        mapped = 'right'
                    elif k2 in (' ',):
                        # space stops continuous movement
                        continuous_dir = None
                        break
                    elif k2 in ('c', 'C'):
                        # toggle continuous: turn off means break
                        continuous_mode = not continuous_mode
                        print('toggle continuous ->', continuous_mode)
                        ...
                        # any other key stops continuous movement
                        continuous_dir = None
                        break
